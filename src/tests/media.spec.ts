...
test.describe("Media Component Happy Path", () => {
    ...
    test("displays video results correctly and video appears when thumbnail clicked", async ({ page }) => {
      // Go to the page, fill form, and submit
      await page.goto("/");
      ...
      await page.waitForSelector('img[aria-label="Video preview thumbnail"]', { timeout: 5000 });
      const videoThumbnails = page.locator('img[aria-label="Video preview thumbnail"]');
