...
test.describe("DescriptionModal Component", () => {
  test("displays preview text correctly and truncates long descriptions", async ({
    page,
  }) => {
    ...
    const textContent = (await previewContainer.textContent())!;
  ...
  test("displays formatted transcript for audio media", async ({ page }) => {
    ...
    const textContent = (await firstSegment.textContent())!;
